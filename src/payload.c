 ...
 */
static int
smp_fetch_ssl_ciphers(const struct arg *args, struct sample *smp, const char *kw, void *private)
{
       int hs_len, ext_len, bleft;
       struct channel *chn;
       unsigned char *data;
 ...
 * message. The format of the message is the following (cf RFC5246 + RFC6066) :
 * TLS frame :
 *   - uint8  type                            = 0x16   (Handshake)
 *   - uint16 version                        >= 0x0301 (TLSv1)
 *   - uint16 length                                   (frame length)
 ...
 * handshake message. The format of the message is the following (cf RFC5246 + RFC6066):
 * TLS frame :
 *   - uint8  type                            = 0x16   (Handshake)
 *   - uint16 version                        >= 0x0301 (TLSv1)
 *   - uint16 length                                   (frame length)
         ...
         if (ext_type == 51) { /* Keyshare extension type ID is 51dec */
          ...
          if (keyshare_len < 2 || keyshare_len > hs_len - 6)
           ...
           while (readPosition < keyshare_len) {
                        ...
                        memmove(&smp->data.u.str.bin[2*numberOfKeyshares], &dataPointer[readPosition], 2);
                        numberOfKeyshares++;
                        if (2*numberOfKeyshares > buffer_bin_size)
                             goto not_ssl_hello; /* not enough space to store all keyshare groups --> something is wrong */
...
static struct sample_fetch_kw_list smp_kws = {ILH, {
	...
	{ "req.ssl_ciphers",     smp_fetch_ssl_ciphers,    0,                      NULL,           SMP_T_BIN,  SMP_USE_L6REQ|SMP_USE_L4CLI|SMP_USE_L5CLI|SMP_USE_FTEND },
